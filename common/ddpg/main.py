...
def main():
	...
	A_MAX = 10 #env.action_space.high[0]
	...
	for _ep in range(MAX_EPISODES):
		...
		for r in range(MAX_STEPS):
			...
			#normalized_action = trainer.get_exploration_action(normalized_state)
			action = trainer.get_exploration_action(normalized_state)
			...
			ref = tra_state + ref[4:8]

			# compute the mpc reference
			ref_traj = env.ego_state + ref + env.goal_state
			...
			if done:
				...
				if params['use_wandb']:
					wandb.log({"episode": _ep, "reward": episode_reward})
				break
