...
class MPC(object):
    ...
    def _initDynamics(self,):
        ...
        for k in range(self._N):
            ...
            if k == 0:
                delta_delta_u_k = self.U[:, k]-[0, 0]
            else:
                delta_delta_u_k = self.U[:, k]-self.U[:, k-1]
    ...
    def _initConstraints(self,surr_vehicle1_pos, surr_vehicle2_pos):
        nlp_g = self.nlp_g
        lbg = self.lbg 
        ubg =   self.ubg 

        for k in range(self._N):
            dist1 = (self.X[0, k+1]-surr_vehicle1_pos[0])**2 + (self.X[1, k+1]-surr_vehicle1_pos[1])**2
            nlp_g += [dist1]

            lbg += [self.vehicle.length*2]
            ubg += [np.inf]

            dist2 = (self.X[0, k+1]-surr_vehicle2_pos[0])**2 + (self.X[1, k+1]-surr_vehicle2_pos[1])**2
            nlp_g += [dist2]
    ...
    def solve(self, ref_states, surr_vehicle1_pos,  surr_vehicle2_pos):
        
        nlp_dict, lbg, ubg   = self._initConstraints(surr_vehicle1_pos,  surr_vehicle2_pos)
        # # reload compiled mpc
        #print(self.so_path)
