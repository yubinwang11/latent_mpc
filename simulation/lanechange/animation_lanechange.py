...
class SimVisual(object):
    ...
    def __init__(self, env):
        ...
        #
        # create figure
        self.fig = plt.figure(figsize=(self.world_size, self.world_size/2)) # 20 15
        # and figure grid
        ...
        self.l_surr_v1_outline, = self.ax_2d.plot([], [], 'r', linewidth=5)
        self.l_surr_v2_outline, = self.ax_2d.plot([], [], 'r', linewidth=5)
        self.surr_v1_pos = self.env.surr_v1_pos
        self.surr_v2_pos = self.env.surr_v2_pos
    ...
    def init_animate(self,):
        ...
        self.l_surr_v1_outline.set_data([], [])
        self.l_surr_v2_outline.set_data([], [])


        return self.l_vehicle_pos, self.l_vehicle_pred_traj, \
            self.l_vehicle_outline, self.l_surr_v1_outline, self.l_surr_v2_outline  #self.l_vehicle_front, self.l_vehicle_back, self.l_vehicle_left, self.l_vehicle_right
    ...
    def update(self, data_info):
        ...
        if len(self.ts) == 0:
            ...
            surr_v1_outline = np.array([[-self.vehicle_length/2, self.vehicle_length/2, self.vehicle_length/2, -self.vehicle_length/2,-self.vehicle_length/2,],
                        [self.vehicle_width / 2, self.vehicle_width / 2, - self.vehicle_width / 2, -self.vehicle_width / 2, self.vehicle_width / 2]])
            surr_v1_outline[0,:] += self.surr_v1_pos[0]
            surr_v1_outline[1,:] += self.surr_v1_pos[1]
            self.l_surr_v1_outline.set_data([surr_v1_outline[0, :]],[surr_v1_outline[1, :]])

            surr_v2_outline = np.array([[-self.vehicle_length/2, self.vehicle_length/2, self.vehicle_length/2, -self.vehicle_length/2,-self.vehicle_length/2,],
                        [self.vehicle_width / 2, self.vehicle_width / 2, - self.vehicle_width / 2, -self.vehicle_width / 2, self.vehicle_width / 2]])
            surr_v2_outline[0,:] += self.surr_v2_pos[0]
            surr_v2_outline[1,:] += self.surr_v2_pos[1]
            self.l_surr_v2_outline.set_data([surr_v2_outline[0, :]],[surr_v2_outline[1, :]])
        ...
        return  self.l_vehicle_pred_traj, \
                self.l_vehicle_outline, self.l_surr_v1_outline, self.l_surr_v2_outline #self.l_vehicle_pos, self.l_vehicle_pred_traj, \
